...
class FFmpegProcessor:
    ...
    def prepare_cloud_upload(
        ...
        audio_path: Path | str,
        mode: str,
        output_path: Path | str = "cache/cloud_upload.mp3",
    ) -> Path:
        source = Path(audio_path)
        supported = (
            {".wav", ".mp3", ".m4a", ".aac", ".ogg", ".opus", ".amr", ".spx", ".pcm"}
            if mode == "standard" else {".wav", ".mp3", ".ogg", ".opus"}
        )
        limit = 512_000_000 if mode == "standard" else 100_000_000
        # Keep the original encoding when the service accepts it. Very large
        # base64 requests consume substantial memory, so compress those first.
        if source.suffix.lower() in supported and 0 < source.stat().st_size < min(limit, 150_000_000):
            return source.resolve()
        self.require_available()
        target = Path(output_path)
        target.parent.mkdir(parents=True, exist_ok=True)
        ...
        logger.info("Preparing compressed cloud upload: %s -> %s", source, target)
        try:
            subprocess.run(command, check=True, capture_output=True, text=True, encoding="utf-8")
        except subprocess.CalledProcessError as exc:
            logger.exception("Cloud upload conversion failed")
            detail = (exc.stderr or "").strip()
            raise FFmpegError(f"云端上传文件压缩失败。{detail[-300:]}") from exc
        if not target.is_file() or not 0 < target.stat().st_size < limit:
            raise FFmpegError("压缩后的云端上传文件仍超过服务限制，请先分割录音。")
        return target.resolve()
